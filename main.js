...
function normalizeChatPreviewType(type){
  ...
  if(type === 'payment' || type === 'payment_card' || type === 'redpacket' || type === 'transfer' || type === 'family_card') return 'payment';
  return 'text';
}
...
function normalizePreviewMessage(msg){
  ...
  if(kind === 'payment'){
    try{
      var pay = typeof next.content === 'string' ? JSON.parse(next.content) : next.content;
      if(pay && typeof pay === 'object'){
        return { content: String(pay.summary || pay.note || pay.title || '支付记录'), type: 'text' };
      }
    }catch(e){}
  }
  if(kind === 'text' && typeof next.content === 'string' && next.content.trim().startsWith('{')){
    try{
      ...
      if(parsed && typeof parsed === 'object' && (parsed.summary || parsed.title || parsed.note)){
        return { content: String(parsed.summary || parsed.note || parsed.title), type: 'text' };
      }
    }catch(e){}
  }
