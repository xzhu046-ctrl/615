...
- 短句不是惜字如金。消息条数按聊天设置来：本角色当前范围是 ${msgMin}-${msgMax} 条。根据当前情绪和话题复杂度，在这个范围内灵活选择具体条数；禁止每次都发送相同条数。每条消息保持简短自然，像真人聊天一样，最好不要超过30个字。
  ...
  chatPolicyBundle() {
    return {
      ...
      replyGuide: [
        ...
        '短句不是惜字如金。消息条数按聊天设置里的 min/max 来；根据当前情绪和话题复杂度，在这个范围内灵活选择具体条数；禁止每次都发送相同条数。每条消息保持简短自然，像真人聊天一样，最好不要超过30个字。',
      ...
      formatGuard: [
        ...
        '同一轮多条纯文本请用 messages 数组或多个 JSON 项；默认一气泡一句话。别整坨发，也别把几个意思串成长句。一个 content/text 字段只写一个真实气泡，每条最好不要超过30个字；想多说就自己增加 messages/JSON 项。只有极少数情绪爆发场景，才允许一整段小作文。',
