...
- 短句不是惜字如金。消息条数按聊天设置来：本角色当前范围是 ${msgMin}-${msgMax} 条。可以在这个范围里多发几条短气泡，有接话、有补刀、有动作、有推进；每个气泡优先 8-22 个汉字，最多别超过 30 个汉字；不要每轮都只回两句，也不要只给礼貌安抚。
  ...
  chatPolicyBundle() {
    return {
      ...
      replyGuide: [
        ...
        '短句不是惜字如金。消息条数按聊天设置里的 min/max 来；可以在设置范围里多发几条短气泡，有接话、有补刀、有动作、有推进；每个气泡优先 8-22 个汉字，最多别超过 30 个汉字；不要每轮都只回两句，也不要只给礼貌安抚。',
        ...
        '一个 JSON 的 content/text 字段就是手机里一个真实气泡。不要把两个以上可以独立发送的短句塞进同一个 content/text 里；如果想连续说，就由你自己输出 messages 数组或多个 JSON 项，每条都短，不要等系统事后拆。',
        '不要车轱辘话。最近几轮已经说过的安抚句、解释理由、反问方式、口头禅不要原样换皮再来一遍；这一轮必须给新的反应、新的动作或新的关系推进。',
      ...
      formatGuard: [
        ...
        '同一轮多条纯文本请用 messages 数组或多个 JSON 项；默认一气泡一句话。别整坨发，也别把几个意思串成长句。一个 content/text 字段只写一个真实气泡，每条优先 8-22 个汉字；想多说就自己增加 messages/JSON 项。只有极少数情绪爆发场景，才允许一整段小作文。',
