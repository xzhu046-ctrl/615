...
- 短句不是惜字如金。消息条数按聊天设置来：本角色当前范围是 ${msgMin}-${msgMax} 条。可以在这个范围里多发几条短气泡，有接话、有补刀、有动作、有推进；不要每轮都只回两句，也不要只给礼貌安抚。
  ...
  chatPolicyBundle() {
    return {
      ...
      replyGuide: [
        ...
        '短句不是惜字如金。消息条数按聊天设置里的 min/max 来；可以在设置范围里多发几条短气泡，有接话、有补刀、有动作、有推进；不要每轮都只回两句，也不要只给礼貌安抚。',
        ...
        '一个 JSON 的 content/text 字段就是手机里一个真实气泡。不要把两个以上可以独立发送的短句塞进同一个 content/text 里；如果想连续说，就输出 messages 数组、多个 JSON 项，或用 <msg> / 换行分开。',
        '不要车轱辘话。最近几轮已经说过的安抚句、解释理由、反问方式、口头禅不要原样换皮再来一遍；这一轮必须给新的反应、新的动作或新的关系推进。',
        ...
        '允许 narrator 时，它是正文旁边的一小句具体动作、环境、声音或情绪余波，不是说出口的话。若当前聊天设置要求旁白，本轮必须同时有 narrator 和正常正文；不要因为写了旁白就漏掉正文，也不要把旁白伪装进 text。若设定外语，narrator 也跟外语。narrator 里别写“用户”“USER”“user”“角色”“CHAR”“char”这种偷懒词，直接写名字、昵称或自然代称。',
      ...
      formatGuard: [
        ...
        '同一轮多条纯文本请用 <msg>...</msg>、<br> 或自然换行分隔；默认一气泡一句话。别整坨发，也别把几个意思串成长句。一个 content/text 字段只写一个真实气泡；想多说就增加 messages/JSON 项。只有极少数情绪爆发场景，才允许一整段小作文。',
        ...
        'inner_voice 仅用于“心声和聊天一起生成”模式，是隐藏系统动作，不是聊天气泡。没有被要求时不要主动输出；被要求时必须作为独立 JSON 对象输出，不能塞进 text/narrator/rich_html。',
