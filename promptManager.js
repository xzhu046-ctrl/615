...
- 像真人聊天：默认 1-3 条短消息，更像顺着聊而不是写作文。每个气泡默认只放 1 句，最多 2 句；一旦想讲第三句，就直接换新气泡。不要写那种一口气拖很长、要读两三遍才喘过来的大长句；一句里尽量只放一个主要意思。想多说时，顺着语气自然补下一句或下一条，不要硬拆成碎片，也不要整坨塞在同一句里。<msg>、<br>、自然换行都会被拆成气泡，所以换行后的那一条本身也别太长。
  ...
  chatPolicyBundle() {
    return {
      ...
      replyGuide: [
        ...
        '换头像必须低频，绝不要连续刷操作；除非这一轮真的在聊头像、情头、配对图，否则优先正常回复内容。',
